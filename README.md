...
- Test262 status as of 2026-05-02:
  ...
  - `language` is not green yet. Recent focused slices have cleared intended failures for logical assignment, compound assignment, and `switch`; loop statement slices are now mostly parser tails (`for`: `4` to `1`, `for-in`: `5` to `2`, `for-of`: `58` to `3`). Remaining active language work is tracked in `plan/test262-language-fix-tracker.md`.
