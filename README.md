...
- Test262 status as of 2026-05-02:
  ...
  - `language` is not green yet. Recent focused slices have cleared intended failures for logical assignment, compound assignment, and `switch`; the focused `for-of` slice is reduced from `58` to `3` failing files, and the adjacent `for-in` slice is reduced from `5` to `2` parser-shape files. The remaining `for-of` tail is two TypeScript parser-shape cases plus one explicit-resource-management case that is outside current scope. Remaining active language work is tracked in `plan/test262-language-fix-tracker.md`.
