...
- Test262 status as of 2026-05-02:
  ...
  - `language` is not green yet. The latest full language scan records `8006` failing files: `1630` intended-scope and `6376` out-of-scope. Recent focused slices cleared intended failures for logical assignment, compound assignment, `switch`, `if`, `try`, `while`, `do-while`, duplicate function parameters, and generator/async `yield` / `await` early errors; the broad intended early-error bucket is now `134`.
