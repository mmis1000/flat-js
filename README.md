...
- Test262 status as of 2026-05-04:
  ...
  - `language` is not green yet. The latest full language scan records `6053` failing files: `15` intended-scope and `6038` out-of-scope. The remaining intended-scope files are TypeScript parser-delegation gaps where valid script syntax is rejected or parsed into the wrong shape: Unicode 16/17 identifiers, sloppy `let` loop-head identifier forms, escaped contextual keywords, `await` as a global binding identifier, and one `for-of` destructuring initializer grammar case. There are no currently known actionable runtime-semantic or supported early-error failures in the full language scan. One object generator-method file crashes the TypeScript 6 checker while collecting semantic diagnostics; treat that as an upstream TypeScript crash to report/defer, not as a Flat JS parser/checker workaround target. The remaining comment/RegExp timeout files are exhaustive Test262 stress loops over 65,536 dynamic eval compilations; representative semantic samples pass, so they are tracked as stress/performance-deferred rather than runtime cleanup targets.
