...
- Test262 status as of 2026-05-02:
  ...
  - `language` is not green yet. Recent focused slices have cleared intended failures for logical assignment, compound assignment, `switch`, `while`, and `do-while`; loop statement slices are now mostly parser tails (`for`: `4` to `1`, `for-in`: `5` to `2`, `for-of`: `58` to `3`, `while`: `9` to `0`, `do-while`: `9` to `0`). A compiler-only parse-negative refresh shows `225` actionable early-error files remain, with `39` parser-delegation exceptions documented in `plan/test262-language-fix-tracker.md`.
