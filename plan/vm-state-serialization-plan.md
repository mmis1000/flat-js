...
14. Phase 7E: add host-boundary guardrails. Implemented 2026-05-05.
...
Status: Phase 7A through 7E implemented 2026-05-05. First version is VM-owned only. Native pending promises and host thenables remain unsupported in snapshots.
...
1. **Phase 7A: scheduler core**
   - Implemented `VmAsyncSession`, VM promise records, FIFO reaction jobs, deterministic timers, and `vmSleep(ticks)`.
   - The VM-owned `Promise` and `vmSleep` are installed only through the optional session path exported by `src/serialization.ts`.
   - Acceptance covered: `vmSleep(1).then(() => { debugger }).then(() => log('end'))` pauses in the first reaction job; `end` logs only after resume.
...
3. **Phase 7C: global debugger pause in web/runtime stepping**
   - Implemented session helpers that return the current debug execution: paused job execution first, otherwise active job, otherwise main execution.
   - The serialization playground now uses async session mode for `async` / `vmSleep` sources, advances virtual timers deterministically, and reads debug stack/highlight/REPL state from the session execution.
   - Acceptance covered: a debugger pause inside a promise job exposes the callback execution as `debugExecution` and resumes the same job.
   - Async session snapshots remain blocked with an explicit error until Phase 7D.
   - Validation run: focused scheduler Jest, existing async/serialization Jest, `npm run build:tsc`, and `npm run build-example:serialization-playground`.
...
5. **Phase 7E: host boundary guardrails**
   - Implemented session-snapshot rejection for native pending promises, native promise `.then` results, host thenables, and async generators.
   - Added a narrow host-promise policy interface for future phases; the current policy remains strict unsupported and turns awaited host thenables into rejected VM promises.
   - Acceptance covered: even registered native `Promise` objects reject in VM async session snapshots; a native `Promise.resolve().then(vmFunc)` result rejects instead of becoming a scheduler job; registered host thenables reject without invoking a `then` getter; async generator state remains unsupported.
   - Validation run: focused scheduler Jest, existing async/serialization Jest, and `npm run build:tsc`.
