(function () { 
    ...
    var ops = {
        ...
        call(opcodes, index, skip) {
            return ({scope, callStack, stack, leave, enter, jump, getInfo, currentIndex})=>{
                var args_length = stack.pop()
                var args = []
                while (args_length--) {args.unshift(stack.pop())}
    ...
    function run(scope, self, fn, index, args) {
        ...
        while (baked[ctx.currentIndex]) {
            ...
            // console.log("before",pad(callStack.length, 4), pad(codes[ctx.currentIndex], 12), ctx.currentIndex, showStack(ctx.stack))
            baked[ctx.currentIndex](ctx)
            // console.log("after ",pad(callStack.length, 4), pad(codes[ctx.currentIndex], 12),ctx.currentIndex, showStack(ctx.stack))
