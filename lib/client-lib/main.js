(function () { 
    ...
    var ops = {
        ...
        call(opcodes, index, skip) {
            return ({scope, stack, leave, enter, jump, getInfo, currentIndex})=>{
                ...
                var info = getInfo(fn) // whether it is internal?
                ...
                if (!info) {
                    ...
                    while (args.length) {stack.push(args.shift())}
    ...
    function run(scope, self, fn, index, args) {
        ...
        while (baked[ctx.currentIndex]) {
            ...
            // console.log("before",pad(codes[ctx.currentIndex], 12), ctx.currentIndex, showStack(stack))
            baked[ctx.currentIndex](ctx)
            // console.log("after ",pad(codes[ctx.currentIndex], 12),ctx.currentIndex, showStack(stack))
